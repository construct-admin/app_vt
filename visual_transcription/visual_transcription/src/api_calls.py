...
def analyze_image_gpt4_resized(image_bytes, prompt):
    ...
    """
    system_prompt = ("You accept images to generate description or alt text "
                     "according to WCAG 2.2 AA accessibility standards.")
    try:
    ...
    messages = [
        ...
        {"role": "user", "content": (
            "Please analyze this compressed image data and provide me with a short visual transcript in no more than 20 words:\n" +
            image_base64
        )}
    ]
...
if st.session_state.uploaded:
    frame_number = st.slider('Select frame', 0, st.session_state.total_frames - 1, st.session_state.frame_number, key="frame_slider")
            ...
            if st.button(f"Transcribe frame {frame_index}", key=f"btn_{frame_index}"):
                ...
                if ret2:
                    ...
                    if response is not None:
                        ...
                        st.error("No response from GPT API.")
                else:
